define([
	...
	"_s",
	"Dialogs",
	"./category_field_configurator",
	"text!./templates/field_picker_dialog_body.html",
	"text!./templates/field_picker.html"
		],
function($, Backbone, _, ui, _s, Dialogs, CategoryFieldConfiguratorView, body_template, field_picker_template){

	CategoryFieldPickerDialogView = Dialogs.views.ModalDialogView.extend({
		events: {
		...
		onSelectedFieldChange: function(){
			...
			this.field_configurator_view = new CategoryFieldConfiguratorView({
		...
		onSave: function(){
			...
			var chart_category_fields = chart_model.get('category_fields');
			if (! chart_category_fields.get(this.configured_field_model.id)){
				chart_model.get('category_fields').add(this.configured_field_model);
				this.hide();
			}
			else{
	...
	return CategoryFieldPickerDialogView;
});
